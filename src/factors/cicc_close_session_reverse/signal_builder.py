...
def read_parquet_with_retry(path, label: str, attempts: int = 4, delay_seconds: float = 0.5) -> pd.DataFrame:
    """容忍短暂写入冲突，并在 PyArrow 不兼容时回退到 DuckDB。"""
    import time
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            last_error = exc
            # 该错误通常来自 PyArrow 对现有 Parquet 编码的兼容问题，
            # 重试不会改善，因此直接交给 DuckDB 读取同一文件。
    ...
    warnings.warn(
        f"pandas 无法读取 {label}，改用 DuckDB: {last_error}",
        RuntimeWarning,
        stacklevel=2,
    )
    try:
        ...
        raise RuntimeError(
            f"{label} 无法读取: {path}；"
            f"pandas={last_error}；duckdb={duckdb_error}"
        ) from duckdb_error
...
def load_inputs() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if missing_files:
        raise FileNotFoundError(
            "缺少 Data_Input_and_Quality.ipynb 产出的 validated parquet: "
            + "; ".join(missing_files)
        )
...
def load_roll_monitor() -> pd.DataFrame:
    ...
    try:
        ...
        raise RuntimeError(
            "无法只读打开 treasury_futures.duckdb；"
            "请关闭正在占用 DuckDB 的 Notebook 或 Python 进程后重试。"
        ) from exc
    try:
        ...
        raise RuntimeError(
            "DuckDB 中不存在 v_roll_migration_monitor；请先运行 "
